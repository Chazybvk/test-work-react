;(function () {
  ...
  function spinnerCode(diameter, color, direction, period, fadeDuration) {
    ...
    element.firstElementChild.animate(
      [{ transform: "rotate(0turn)" }, { transform: "rotate(1turn)" }],
      {
        ...
        iterations: Infinity,
        direction: direction === "counter-clockwise" ? "reverse" : "normal",
      }
    )
    return element
  }
  ...
  function get_element_to_wait_for(element, previous) {
    return element.dataset.await !== undefined
  ...
  function play() {
    ...
    getEmergeElements().forEach(function (self, index, emerging) {
      if (
        ...
        throw `Emerge: Element with ID “${self.dataset.await}” not found.`
      }
      ...
      first.push(
        Promise.all(
          [self]
            .concat(Array.from(self.querySelectorAll("*")))
            .reduce(function (sources, element) {
              if (element.nodeName.toLowerCase() === "img") {
                ...
                sources.push(
                  eventDispatched(
                    element,
                    element.dataset["emerge-event"] || "canplaythrough"
                  )
                )
              }
      ...
      if (element_to_wait_for !== undefined && !is_cyclic(self)) {
        first.push(
      ...
      if (self.dataset.spin) {
        ...
        if (customSpinner !== null) {
          ...
          const spinnerOptions = Object.keys(spinner_defaults).reduce(function (
            options,
            key
          ) {
            options[key] =
